...
data FeatureSpec a = FS { label    :: Tok a -> Smth a
...
predict beamSize models sents = map predictOne sents
    where predictOne s = fst . head . beamSearch beamSize models $ [(fromList s,1)]

predictPipeline :: Int -> [Model a] -> [[Tok a]] -> [[Tok a]]
predictPipeline beamSize models sents = map predictOne sents
    where predictOne s = foldl (\s1 m -> fst . head . beamSearch beamSize [m] $ [(fromList s1,1)]) s models
                               

type Label a = Smth a
sentToExamples ::  FeatureSpec a -> [Tok a] -> [(Label a,[Feature String Double])]
