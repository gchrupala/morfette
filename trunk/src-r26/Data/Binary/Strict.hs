...
--
-- This is an hack to use the strict version of the Get monad from the binary-strict package.
-- By Grzegorz Chrupala <grzegorz.chrupala@computing.dcu.ie>
--
-----------------------------------------------------------------------------
...
--getWord16be
decode :: Binary a => ByteString -> a
decode s = case runGet get (B.concat (L.toChunks s)) of
             (Right v, _) -> v
             (Left err,_) -> error ("Data.Binary.Strict.decode: " ++ err)
...
--
lazyToStrict :: ByteString -> B.ByteString
lazyToStrict = B.concat . L.toChunks
strictToLazy :: B.ByteString -> ByteString
strictToLazy = L.fromChunks . return
instance Binary ByteString where
      put    = put . lazyToStrict
      get    = do s <- get 
                  return (strictToLazy s)
